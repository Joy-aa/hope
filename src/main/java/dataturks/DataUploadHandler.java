...
import dataturks.response.HitResult;
...
public class DataUploadHandler {
    ...
    private static UploadResponse handleDocumentAnnotationSingleFile(DReqObj reqObj, DProjects project, String filePath) {
        ...
        if (reqObj.getUploadFileType() == DTypes.File_Upload_Format.PRE_TAGGED_JSON) {// 处理包含预标记的JSON文件
    ...
    private static UploadResponse handleTasksWithPreTaggedJSONFile(DReqObj reqObj, DProjects project, String filePath) {
        ...
        try {
            jsonArray = UploadFileUtil.readPreTaggedFromText(filePath);
        } catch (Exception e) {
        ...
        for (int i=0; i<objects.size(); i++){
            JSONObject node = (JSONObject) objects.get(i);// 获取单个节点
...
//            }
            ...
            String hitData = node.getString("dataUrl");// 图片URL
            String hitExtras = node.getString("extras");
            JSONObject hitResultData = (JSONObject) node.getJSONArray("hitResults").get(0);// hitResults
            ...
            if (Validations.isValidDataItemForTextTask(hitData, reqObj)) {
                ...
                if (hitResultData != null) {
                    ...
                    hitsResult.setResult(hitResultData.getString("result"));
                    hitsResult.setModel(hitResultData.getString("predLabel"));
                    hitsResult.setModel(hitResultData.getString("model"));
                    hitsResult.setStatus(DConstants.HIT_STATUS_PRE_TAGGED);
	...
	LOG.info("urls:"+urls);
        if (urls != null && !urls.isEmpty()) {
			...
			LOG.info("shouldValidateUrl:"+shouldValidateUrl+",DUtils.isValidURL(url):"+DUtils.isValidURL(url));
                    continue;
                }

                if (response.getNumHitsCreated() > reqObj.configs.getNumHitsPerProject()) {
			LOG.info("reqObj.configs.getNumHitsPerProject:"+reqObj.configs.getNumHitsPerProject());
                    break;
                }

                try {
                    if (createHitWithContent(reqObj, project, url, false, true) != null) {
			    LOG.info("incrementHits\n");
                        response.incrementHits();
                    } else {
			    LOG.info("incrementIgnored\n");
                        response.incrementIgnored();
                    }
                } catch (Exception e) {
    ...
    private static UploadResponse handleImageTasks(DReqObj reqObj, DProjects project, String filePath) {
        ...
        DTypes.File_Type type = UploadFileUtil.getFileType(filePath);// 获取上传的文件类型
        ...
        if (type == DTypes.File_Type.IMAGE) {
            ...
            String filePathOut = MrxsDealUtil.mrxsTrans(filePath, uploadDir, folderPath.toString());  //MRXS文件处理
            ...
            filePath = ImageTransUtil.trans(filePath, uploadDir);
            return handleImageTasksWithSingleImageFile(reqObj, project, filePath);
        } else if (type == DTypes.File_Type.NII || type == DTypes.File_Type.GZ) {
            List<String> imageFiles = NiiTransUtil.trans(filePath, uploadDir);
            List<String> urls = DataStorageHandler.uploadAndGetURLs(imageFiles, project);
