...
public class DProjectsDAO extends AbstractDAO<DProjects> implements IDDao<DProjects>{
    ...
    // 查询所有的公共数据集
    private List<DProjects> findAllPublic() {
        return list(namedQuery("bonsai.dropwizard.dao.d.DProjects.findAllPublic"));
    }

    //Called from within the app and not via a hibernate resources hence does not have session binding.
    public List<DProjects> findAllPublicInternal() {
        Session session = sessionFactory.openSession();
        try {
            ManagedSessionContext.bind(session);
            Transaction transaction = session.beginTransaction();
            try {
                List<DProjects> list= findAllPublic();
                transaction.commit();
                return list;
            }
            catch (Exception e) {
