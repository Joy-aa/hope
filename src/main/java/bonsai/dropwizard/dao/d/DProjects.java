...
@NamedQueries({
        ...
        @NamedQuery(name = "bonsai.dropwizard.dao.d.DProjects.findAllPublic",
                query = "select e from DProjects e where status != 'DELETED' and isPublic = '1'"),
...
public class DProjects implements IDdbPojo{
    ...
    // 医学图像格式和器官类型，定义数据库字段类型为 text
    @Column(columnDefinition = "text")
    private String medicalImageFormat;
    @Column(columnDefinition = "text")
    private String imageOrganType;
    ...
    public DProjects(String name, String orgId){
        this.name = name;
        this.orgId = orgId;
        // 创建项目时默认取当前时间，不然时间戳部分为NULL，会引起前端不显示项目
    ...
    public String getMedicalImageFormat() {
        return medicalImageFormat;
    }

    public void setMedicalImageFormat(String medicalImageFormat) {
        this.medicalImageFormat = medicalImageFormat;
    }

    public String getImageOrganType() {
        return imageOrganType;
    }

    public void setImageOrganType(String imageOrganType) {
        this.imageOrganType = imageOrganType;
    }
}